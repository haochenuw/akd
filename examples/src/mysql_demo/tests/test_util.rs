...
#[macro_export]
macro_rules! directory_test_suite_macro {
    ($proof:ident, $verify:ident) => {
        paste::paste! {
        pub(crate) async fn [<directory_test_suite _ $verify>]<
            TC: Configuration,
            S: Database + 'static,
            V: VRFKeyStorage,
        >(
            ...
            match maybe_dir {
                ...
                Ok(dir) => {
                    ...
                    for user in users.iter().choose_multiple(&mut rng, 2) {
                        let key = AkdLabel::from(user);
                        match dir.$proof(&key, HistoryParams::default()).await {
                            Err(error) => {
                                panic!("Error performing key history retrieval {:?}", error)
                            }
                            Ok((proof, root_hash)) => {
                                let vrf_pk = dir.get_public_key().await.unwrap();
                                if let Err(error) = akd::client::$verify::<TC>(
                                    vrf_pk.as_bytes(),
                                    root_hash.hash(),
                                    root_hash.epoch(),
                                    key,
                    ...
                    match dir.audit(1u64, 2u64).await {
                        ...
                        Ok(proof) => {
                            ...
                            akd::auditor::audit_verify::<TC>(
                                vec![start_root_hash, end_root_hash],
                                proof,
                            )
                            .await
                            .unwrap();
...
directory_test_suite_macro!(key_history, key_history_verify);
directory_test_suite_macro!(key_history_v2, key_history_verify_v2);
